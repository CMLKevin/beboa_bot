---
title: User Memory
description: Vector-based per-user long-term memory using text embeddings.
---

# User Memory System

Beboa uses vector embeddings for per-user long-term memory, enabling contextual recall of facts about individual users.

> **See Also**: For server-wide awareness of all messages, see [Server-Wide Memory](/ai/server-memory).
...
## User Memory vs Server Memory

| Feature | User Memory | Server Memory |
|---------|-------------|---------------|
| Scope | Per-user facts | All server messages |
| Source | AI-extracted facts | Raw message content |
| Storage | `memories` table | `server_messages` table |
| Retrieval | User-specific search | Server-wide semantic search |
| Purpose | Remember user preferences | Ambient conversation awareness |

Both systems complement each other - User Memory stores personal facts, while [Server Memory](/ai/server-memory) provides conversational context.
