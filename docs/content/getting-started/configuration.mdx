---
title: Configuration
description: Complete reference for all Beboa environment variables.
---
...
# LLM Evaluator (mood, relationships, Jarvis parsing)
...
| LLM Evaluator | `x-ai/grok-4.1-fast` | $ |
...
SERVER_MEMORY_EXCLUDED_CHANNELS=123456789,987654321
...
See [Server-Wide Memory](/ai/server-memory) for full documentation.
...
### `LLM_EVALUATOR_ENABLED`
Enables the LLM-powered evaluation system for mood detection, relationship assessment, and Jarvis intent parsing. When disabled, falls back to keyword-based logic.

### `LLM_EVALUATOR_CACHE_TTL`
How long (in milliseconds) to cache LLM evaluation results. Helps reduce API calls for repeated interactions.

### `LLM_EVALUATOR_RATE_LIMIT`
Maximum LLM evaluator calls per minute. Prevents excessive API usage during high activity.
